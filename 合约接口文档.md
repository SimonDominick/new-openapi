...
| Signature        | y        | string | 签名                                     |          |                                                              |
| Timestamp        | y        | string | 时间戳                                   |          |                                                              |
| type             | n        | string | 类型                                     |          | 1:币本位，0:金本位                                           |
| stopLimitType    | n        | int    | 条件单类型（存在条件单时有值，否则为空） |          | 0 -止盈 1-止损 2-计划委托                                    |
...
| page             | n        | int    | 页码                                     |          |                                                              |
| pageSize         | n        | int    | 大小                                     |          |                                                              |
| startDate        | n        | string | 完成订单开始时间（包含开始时间）         |          | 毫秒Long                                                     |
| endDate          | n        | string | 完成订单结束时间（包含结束时间）         |          | 毫秒Long                                                     |
| status           | n        | string | 订单成交状态类型                         | 0,1,2,-1 | 0 等待成交 1 部分成交 2 已经成交 -1 已经撤销                 |
